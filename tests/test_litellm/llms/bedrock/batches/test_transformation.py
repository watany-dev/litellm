...
def test_cancel_request_builds_stop_url_with_empty_signed_body(config):
    with patch.object(config.common_utils, "sign_aws_request") as mock_sign:
        mock_sign.return_value = ({"Authorization": "signed"}, b"")
        result = config.transform_cancel_batch_request(batch_id=ARN, optional_params={}, litellm_params={})
    assert result["method"] == "POST"
    assert result["headers"] == {"Authorization": "signed"}
...
def test_cancel_request_rejects_async_invoke_arn(config):
    ...
    with pytest.raises(ValueError, match="async-invoke"):
        config.transform_cancel_batch_request(batch_id=async_arn, optional_params={}, litellm_params={})


def test_cancel_request_rejects_non_arn(config):
    with pytest.raises(ValueError, match="Expected ARN"):
        config.transform_cancel_batch_request(batch_id="abc1234567", optional_params={}, litellm_params={})


def test_cancel_request_rejects_bad_region(config):
    bad = "arn:aws:bedrock:US_WEST:123:model-invocation-job/x"
    with pytest.raises(ValueError, match="Invalid region in ARN"):
        config.transform_cancel_batch_request(batch_id=bad, optional_params={}, litellm_params={})
